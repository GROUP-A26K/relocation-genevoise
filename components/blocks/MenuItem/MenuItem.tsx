    ...
    return (
      <NavigationMenuItem
        ...
        className={cn(
          ...
          "focus:!bg-transparent focus:!text-primary-500 focus:border-b-4 focus:!border-b-secondary-500"
        )}
      >
        <NavigationMenuTrigger
          className={cn(
            ...
            "hover:text-accent-foreground hover:bg-transparent hover:!text-primary-500 hover:border-b-[3px] hover:!border-b-primary-500",
            "focus:!bg-transparent focus:!text-none  focus:!none",
            "active:!bg-transparent active:!text-primary-500 active:!border-b-primary-500",
            "data-[state=open]:hover:bg-transparent data-[state=open]:!text-primary-500 data-[state=open]:!border-b-primary-500"
          )}
        >
          {item.title}
        </NavigationMenuTrigger>
        ...
        >
          <div className="flex justify-center w-full m-0">
            <div className="!w-full flex xl:flex-row flex-col 2xl:max-w-screen-2xl xl:max-w-screen-xl lg:max-w-screen-xl md:max-w-screen-md  md:w-[300px] lg:w-[400px]  xl:px-[100px] lg:px-[48px] px-[50px]">
              {/* SubMenuLink */}
              {/* <div className="flex flex-row gap-x-12 p-6 pt-0 pl-0">
                {item.items.map((subItem) => {
                  return (
                    ...
                    >
                      <p className="text-[12px] font-bold uppercase px-[12px] !leading-[130%]">
                        {subItem.title}
                      </p>

                      <div
                        className={cn(
                          'grid gap-y-3 gap-x-12 grid-cols-2',
                          subItem?.items?.length == 3 && 'grid-cols-1'
                        )}
                      >
                        {subItem?.items &&
    ...
    <li key={item.title}>
      <Link
        className={cn(
          ...
          "hover:text-accent-foreground hover:text-primary-500 hover:border-b-[3px] hover:border-b-secondary-500"
        )}
        href={item.url}
      >
        {item.title}
