...
export const FALLBACK_RATES: IExchangeRates = {
  CHF: 1,
  EUR: 1.107,
  USD: 1.123,
};

export const getCachedRates = (): IExchangeRates | null => {
  ...
  try {
    ...
    const cached = JSON.parse(raw) as TCachedRates;
...
export const setCachedRates = (rates: IExchangeRates): void => {
  if (typeof document === 'undefined') return;

  try {
    const payload: TCachedRates = { rates, fetchedAt: Date.now() };
