...
type TMarker = 'bold' | 'italic' | 'link' | 'strong';
...
function findNextMarker(
  ...
  const entries: Array<[number, TMarker]> = [
    [link, 'link'],
    [bold, 'bold'],
    [italic, 'italic'],
    [strong, 'strong'],
  ].filter(([pos]) => pos !== -1) as Array<[number, TMarker]>;
...
function parseSegment(segment: string, keyPrefix: string): ReactNode[] {
  ...
  while (cursor < segment.length) {
    ...
    if (end === -1) {
      nodes.push(
        <React.Fragment key={`${keyPrefix}-${key++}`}>{open}</React.Fragment>
      );
      cursor = startInner;
      continue;
    }
...
export const FormattedText: React.FC<IFormattedTextProps> = ({ text }) => {
